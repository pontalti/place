...
public class Place {
     ...
     * forces the lazy collection to initialise on every comparison and changes
     * the hash once the id is assigned on persist — enough to lose the entity
     * inside a HashSet. A constant hash is the standard workaround: correct
     * before and after persist, at the cost of degrading hash buckets for the
     * handful of entities a single session holds.
     *
     * <p>{@code Hibernate.getClass} unwraps a proxy, so a loaded entity and a
     * proxy of the same row compare equal.
     */
    @Override
